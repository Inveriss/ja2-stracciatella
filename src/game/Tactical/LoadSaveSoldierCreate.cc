...
// ---------------------------------------------------------------------------
// Legacy (vanilla) format: frozen at the original 4-attachment-slot OBJECTTYPE
// layout. Used only for sector map files (the ones shipped with the base game,
...
void ExtractLegacySoldierCreateFromFile(HWFILE const f, SOLDIERCREATE_STRUCT* const c, bool stracLinuxFormat)
{
	if(stracLinuxFormat)
	{
		BYTE data[1060];
		f->read(data, sizeof(data));
		ExtractLegacySoldierCreate(data, c, stracLinuxFormat);
	}
	else
	{
		BYTE data[1040];
...
static void InjectLegacySoldierCreate(BYTE* const data, const SOLDIERCREATE_STRUCT* const c)
{
	DataWriter d{data};
	INJ_BOOL(d, c->fStatic)
	INJ_U8(d, c->ubProfile)
...
void InjectLegacySoldierCreateIntoFile(HWFILE const f, SOLDIERCREATE_STRUCT const* const c)
{
	BYTE data[1040];
	InjectLegacySoldierCreate(data, c);
	f->write(data, sizeof(data));
}
